...
export default function ArchivePage() {
  ...
  const handleUnarchive = async (r: Rec) => {
    const ok = await confirm({
      ...
      message: `Restore "${r.documentTitle}" to active records?`,
      confirmText: "Restore",
      variant: "info",
    });
    if (!ok) return;
    try {
      await unarchiveRecord(r.id).unwrap();
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-gray-500 text-xs mt-0.5">
            {meta?.total ?? 0} record{meta?.total !== 1 ? "s" : ""} in archive
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <div className="flex gap-1 bg-gray-900 border border-white/5 rounded-xl p-1 sm:shrink-0">
        ...
        {isLoading ? (
          ...
          <div className="py-20 text-center">
            ...
            <p className="text-gray-600 text-xs mt-1">
              {search || status ? "Try adjusting your filters" : "No records have been archived yet"}
            </p>
          </div>
        ) : (
          <div className="divide-y divide-white/[0.04]">
            {records.map(r => (
              <div
                key={r.id}
                className={`group transition-colors ${selected.has(r.id) ? "bg-blue-500/5" : "hover:bg-white/[0.02]"}`}
              >
                {/* Desktop */}
                <div className="hidden sm:grid grid-cols-12 gap-4 items-center px-5 py-3.5">
                  ...
                  <div className="col-span-4 min-w-0">
                    ...
                    {r.documentNumber && (
                      <p className="text-gray-500 text-xs mt-0.5">#{r.documentNumber}</p>
                    )}
                  ...
                  <div className="col-span-2">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${TYPE_COLOR[r.type]}`}>
                      {r.type}
                    </span>
                  </div>
                  <div className="col-span-2">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${STATUS_COLOR[r.status]}`}>
                      {r.status}
                    </span>
                  </div>
                ...
                <div className="sm:hidden px-4 py-3.5">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-gray-500 text-xs mt-0.5">{r.personName}</p>
                    ...
                    <div className="flex gap-1.5 flex-col shrink-0">
                      <button
                        onClick={() => handleUnarchive(r)}
